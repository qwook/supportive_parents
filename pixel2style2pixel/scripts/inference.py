...
def run_on_batch(inputs, net, opts):
    ...
    if opts.latent_mask is None:
        result_batch = net(inputs, randomize_noise=False, resize=opts.resize_outputs, apply_direction=True, direction=(direction * coeff) + (direction2 * coeff2))
    else:
        ...
        for image_idx, input_image in enumerate(inputs):
            ...
            for j in range(len(latent_to_inject)):
                latent_to_inject[j][0:8] = \
                    (latent_to_inject[j] + coeff * direction)[0:8]
